...
app.get('/', async (c) => {
  ...
  const themeName = c.req.query('theme') ?? c.env.THEME ?? 'default';
  const theme = resolveTheme(themeName);
  // ステータスページは短時間キャッシュし、D1読み取りとレンダリングコストを抑える。
  // テーマごとに別キャッシュにする
  const cache = caches.default;
  const origin = new URL(c.req.url).origin;
  const cacheKey = new Request(`${origin}/__cache__/status/${encodeURIComponent(themeName)}`);
  const hit = await cache.match(cacheKey);
  if (hit) return hit;

  const data = await getStatusData(c.env);
  const res = await c.html(renderStatusPage(data, c.env.TIMEZONE ?? 'UTC', theme));
